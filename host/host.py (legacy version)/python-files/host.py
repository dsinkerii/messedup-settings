...
# Change this if you want to connect to a new server.
broker = 'mqtt.eclipseprojects.io'
port = 1883


topic = "1.20settingsmodv1.2"
...
def on_connect(client, userdata, flags, rc, properties):
    if rc == 0:
        ...
        print("Properties:")
        print(properties)
        print("Warning! If code says \"MQTTErrorCode.MQTT_ERR_SUCCESS\" don't be scared!\nIt's just a weird way for mqtt to say that your message is published...")
        pass
    else:
...
client.connect(broker, port, 60) # why 60? idk.
client.loop_start()
...
with dpg.window(label='SettingsMenu',tag="window") as window:
    ...
    dpg.add_text("Ver. " + version + "    Server: tcp://" + broker + ":" + str(port),color=[150,150,155])
    ...
    with dpg.child_window(label="fun stuff", autosize_x=True,parent=window) as child_window_boring:
        ...
        dpg.add_combo(label="Rebind swapOffhand",width=300,tag="swapOffhand",items=keybings,callback=push_settings,user_data=["key_key","swapOffhand"])
